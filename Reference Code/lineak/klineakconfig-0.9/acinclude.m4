...
AC_DEFUN([AC_LINEAK_CHECK_LINEAKKB_DEF],
[
   AC_MSG_CHECKING([for keyboard definitions file])
   AC_CACHE_VAL(lineakkb_def_file,
   [
      lineakkb_def_file=no
      lineakkb_def_file=`which lineakd`;
      if test "$lineakkb_def_file" != ""; then
            lineakkb_def_file=`dirname $lineakkb_def_file`
            if test -r "$lineakkb_def_file/../etc/lineakkb.def"; then
                  lineakkb_def_file="$lineakkb_def_file/../etc/lineakkb.def"
            else
                  lineakkb_def_file=no
            fi
      else
      ...
      if test "$lineakkb_def_file" = "no"; then
        for ac_file in    \
                              \
                  /usr/local \
                  /usr \
        ...
        ; \
        do
                if test -r "$ac_file/etc/lineakkb.def"; then
                        lineakkb_def_file="${ac_file}/etc/lineakkb.def"
                        break
                fi
        done
      fi
      lineakkb_def_file=`echo $lineakkb_def_file | sed 's/\/\//\//g'`
  ])
  AC_MSG_RESULT($lineakkb_def_file)
  if test "$lineakkb_def_file" != "no"; then
    AC_DEFINE_UNQUOTED(LINEAKKB_FILE, "$lineakkb_def_file", [Define the file for keyboard definitions])
  else
    echo The lineakd keyboard definition file lineakkb.def could not be found.
    exit 0
  fi

  AC_DEFINE_UNQUOTED(ROOTDIR, "$prefix", [Define the root of the installation directory])
